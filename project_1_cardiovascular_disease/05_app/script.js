(() => {
  ...
  const explorerTitles = {
    medical: "Medical range comparison",
  ...
  function riskState(probability) {
    if (probability >= 0.6) {
      return {
        ...
        gauge: "#e35d4f",
        message:
          "This profile is clearly above the review point. The checker would suggest professional review because several entered details match groups with higher observed cardiovascular disease rates.",
      };
    }
    ...
    if (probability >= threshold) {
      return {
        level: "medium",
        title: "Risk review suggested",
        pill: "Review suggested",
        gauge: "#d59a1f",
        message:
          "This profile is above the review point. The checker would flag it for follow-up because the project is designed to catch more possible risk cases.",
      };
    }
    ...
    return {
      ...
      gauge: "#00b894",
      message:
        "This profile is below the review point used in the project. That does not mean no health risk exists, but the checker does not flag this profile for review.",
    };
  }
  ...
  function updateResult({ announce = false } = {}) {
    ...
    elements.formMessage.textContent = announce ? "Risk check updated." : "";
  ...
  function resultSummary() {
    ...
    return [
      ...
      `Estimated risk: ${pct(result.probability, 1)}`,
      `Result: ${state.title}`,
